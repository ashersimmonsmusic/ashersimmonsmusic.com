    ...
    <Container className="py-16 md:py-24">
      ...
      <div className="grid gap-12 lg:grid-cols-[1fr_1.3fr] lg:gap-20">
        <div>
          ...
          <dl className="font-mono-label mt-8 space-y-4 text-sea-mist">
        ...
        <div className="space-y-6 text-lg leading-relaxed text-sea-mist">
          {paragraphs.map((paragraph, i) => (
            <p key={i} className={i === 0 ? "font-display text-2xl text-bone md:text-3xl" : undefined}>
              {paragraph}
            </p>
          ))}

          <div className="mt-14">
            <h2 className="font-mono-label mb-6 text-sea-mist">On stage</h2>
            <ul className="hairline grid grid-cols-2 gap-x-6 gap-y-4 border-t pt-6 sm:grid-cols-3">
              {artist.achievements.map((achievement) => (
                <li key={achievement._id}>
                  <p className="font-display text-lg text-bone">{achievement.label}</p>
                  {achievement.detail && (
                    <p className="font-mono-label text-sea-mist">{achievement.detail}</p>
                  )}
                </li>
              ))}
            </ul>
